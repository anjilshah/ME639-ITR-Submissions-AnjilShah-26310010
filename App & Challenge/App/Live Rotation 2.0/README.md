...
The original dashboard is preserved and a new proof section is added for the
single requested property:

R(X × Y) = R(X) · R(Y)

The V2 add-on provides independent live controls for rotations X and Y,
displays R(X), R(Y), their matrix product, and a directly composed R(X × Y).
The two results are compared numerically while the user changes the angles.

The original dashboard controls and behavior are retained.
