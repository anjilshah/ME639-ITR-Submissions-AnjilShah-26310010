...
/* ================================================================
   VERSION 2 ADD-ON — ROTATION COMPOSITION PROPERTY
   Existing V1 JavaScript above is intentionally untouched.
   ================================================================ */

(() => {
  const ids = [
    "v2-x-roll","v2-x-pitch","v2-x-yaw",
    "v2-y-roll","v2-y-pitch","v2-y-yaw"
  ];
  ...
  const fmtV2 = value => Number(value).toFixed(3);

  const v2MatrixMultiply = (a,b) => {
  ...
  const v2RenderMatrix = (id,matrix) => {
    const el = document.querySelector("#" + id);
    if(!el) return;
    el.innerHTML = matrix
      .map(value => `<span>${fmtV2(value)}</span>`)
      .join("");
  };

  const v2Matrix4 = m => new THREE.Matrix4().set(
    m[0],m[1],m[2],0,
    m[3],m[4],m[5],0,
    m[6],m[7],m[8],0,
    0,0,0,1
  ...
  /*
   * Convert the existing dashboard's ZYX Euler angles into a quaternion.
   * Three.js then composes X and Y using quaternion multiplication:
   *
   *     Q(X × Y) = Q(X) Q(Y)
   *
   * and converts that composed orientation back into a matrix.
   *
   * This gives the "direct R(X × Y)" side independently from the
   * displayed matrix multiplication R(X)R(Y).
   */
  const v2QuaternionFromEuler = (roll,pitch,yaw) => {
  ...
  const v2QuaternionMatrix = q => {
    const m4 = new THREE.Matrix4().makeRotationFromQuaternion(q);
    const e = m4.elements;

    // Convert Three.js column-major storage into the row-major
    // 3x3 representation used by the existing dashboard.
    return [
      e[0],e[4],e[8],
      e[1],e[5],e[9],
      e[2],e[6],e[10]
  ...
  const v2RotationMatrix = (roll,pitch,yaw) => {
    ...
    // Same ZYX convention as the original dashboard:
    // R = Rz(yaw) Ry(pitch) Rx(roll)
    return [
      cy*cp, cy*sp*sr - sy*cr, cy*sp*cr + sy*sr,
      sy*cp, sy*sp*sr + cy*cr, sy*sp*cr - cy*sr,
  ...
  const updateV2Proof = () => {
    const xRoll = getV2("v2-x-roll");
    const xPitch = getV2("v2-x-pitch");
    const xYaw = getV2("v2-x-yaw");

    const yRoll = getV2("v2-y-roll");
    const yPitch = getV2("v2-y-pitch");
    const yYaw = getV2("v2-y-yaw");

    const RX = v2RotationMatrix(xRoll,xPitch,xYaw);
    const RY = v2RotationMatrix(yRoll,yPitch,yYaw);

    // Left side of the requested property.
    const product = v2MatrixMultiply(RX,RY);

    // Right/direct side: compose X and Y as rotations using
    // quaternion composition, then convert the result to R(X × Y).
    const qX = v2QuaternionFromEuler(xRoll,xPitch,xYaw);
    const qY = v2QuaternionFromEuler(yRoll,yPitch,yYaw);

    // In Three.js, premultiply gives qX * qY.
    const qXY = qX.clone().multiply(qY);
    const direct = v2QuaternionMatrix(qXY);

    const error = v2MatrixError(product,direct);

    v2RenderMatrix("v2-matrix-x",RX);
    v2RenderMatrix("v2-matrix-y",RY);
    v2RenderMatrix("v2-matrix-product",product);
    v2RenderMatrix("v2-matrix-direct",direct);

    const errorEl = document.querySelector("#v2-error");
    const statusEl = document.querySelector("#v2-status");

    if(errorEl){
      errorEl.textContent = error.toFixed(9);
    }

    if(error < 1e-9){
      if(statusEl){
        statusEl.textContent = "✓ PROPERTY VERIFIED";
        statusEl.className = "v2-status-ok";
      }
    }else{
      if(statusEl){
        statusEl.textContent = "CHECK COMPOSITION";
        statusEl.className = "v2-status-fail";
      }
    }
  };

  ids.forEach(id => {
    document.querySelector("#" + id)?.addEventListener("input",updateV2Proof);
  });
